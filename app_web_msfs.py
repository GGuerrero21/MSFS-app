...
def leer_vuelos():
    ...
    try:
        ...
        # Map any column name variant → canonical name
        col_map = {
            'Fecha': 'Date', 'fecha': 'Date',
            'Origen': 'Origin', 'origen': 'Origin',
            'Destino': 'Destination', 'destino': 'Destination',
            'Num_Vuelo': 'Flight_No', 'Numero_Vuelo': 'Flight_No',
            'Aerolinea': 'Airline', 'Aerolínea': 'Airline',
            'Modelo_Avion': 'Aircraft', 'Modelo_Avión': 'Aircraft',
            'Hora_OUT': 'Block_Out', 'Hora_IN': 'Block_In',
            'Tiempo_Vuelo_Horas': 'Block_Time',
            'Landing_Rate_FPM': 'Landing_FPM',
            'Distancia_NM': 'Distance_NM',
            'Gate_Salida': 'Gate_Dep', 'Gate_Llegada': 'Gate_Arr',
            'Notas': 'Notes', 'Ruta': 'Route',
        }
        df = df.rename(columns=col_map)

        # Drop header-as-row if it slipped through
        if 'Origin' in df.columns:
            df = df[~df['Origin'].isin(['Origen', 'Origin', ''])]
        if 'Destination' in df.columns:
            df = df[~df['Destination'].isin(['Destino', 'Destination', ''])]

        for col in ['Distance_NM', 'Landing_FPM']:
